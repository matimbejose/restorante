...
<form action="?btn=relatoriogarcon&busca=data" method="post" enctype="multipart/form-data" id="form">
	...
	$gar =mysqli_query($connect,"SELECT * FROM garcon WHERE idGarcon !='1' ORDER BY nomeGarcon ASC");
...
$confere = mysqli_query($connect,"SELECT * FROM config");
...
?>
	...
	$query = mysqli_query($connect,"SELECT data, nome, preco, SUM(preco) AS pr, SUM(qtd) AS qtd, date_format(data, '%d/%m/%Y') AS data FROM tbl_carrinho WHERE idGarcon='$garcon' AND data BETWEEN '$datai' AND '$dataf' AND time BETWEEN '$hora1' AND '$hora2' GROUP BY data") or die(mysqli_error());

	

	$sel = mysqli_query($connect,"SELECT * FROM tbl_carrinho")or die(mysqli_error());

		while($resultado = mysqli_fetch_array($query)){

			$ids = $resultado['idGarcon'];

			$oo = mysqli_query($connect,"SELECT * FROM garcon WHERE idGarcon='$garcon'")or die(mysqli_error());
			...
			$comi = mysqli_query($connect,"SELECT * FROM config");
